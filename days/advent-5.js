...
const knownOptCodes = [1, 2, 3, 4, 99];

function processOptCode(memory, index, optcode, input, paramModes) {
    if (optcode === 99) {
        return null;
    } else if (optcode === 1) {
        ...
        const sPosition = memory[index+1];
        memory[sPosition] = input;
        return index + 2;
    } else if (optcode === 4) {
        const wPosition = memory[index+1];
        console.log('[OUT]:_> ', memory[wPosition]);
        return index + 2;
    } else {
...
function intCodeComputer(source, input) {
    ...
    while (index < newSource.length) {
        if (knownOptCodes.indexOf(newSource[index]) !== -1) {
            const newIndexS = processOptCode(newSource, index, newSource[index], input, [0, 0, 0])
            if (newIndexS !== null) {
                index = newIndexS
            } else {
                break;
            ...
            const newIndexC = processOptCode(newSource, index, state.optcode, input, state.paramModes);
            if (newIndexC !== null) {
                index = newIndexC
            } else {
                break;
